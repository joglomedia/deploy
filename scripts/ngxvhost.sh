...
server {
    ...
    location ~ \.php$ {
        ...
        fastcgi_pass unix:/run/php/php5.6-fpm.${UserName}.sock;
    }
...
if [ ! -f "/etc/php/5.6/fpm/pool.d/${UserName}.conf" ]; then
	echo "The FPM pool configuration for user ${UserName} doesn't exist, attempting to add new pool configuration..."

	create_fpm_pool_conf > /etc/php/5.6/fpm/pool.d/$UserName.conf
	touch /var/log/php5.6-fpm_slow.$UserName.log
